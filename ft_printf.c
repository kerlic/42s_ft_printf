...
/*   Updated: 2021/02/09 22:10:30 by ilsong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		print_c_str(char *data, int *flg, char fmt)
{
	...
	{
		nop += 1;
		flg[r_arr] == 0 ? print_data_cstr(data, 0, 'c') : 0;
		nop += (flg[is_wdth]) ? print_space(data, flg[wdth] - flg[lenth], flg, 'c') : 0;
		flg[r_arr] == 1 ? print_data_cstr(data, 0, 'c') : 0;
	}
...
//	temp = NULL;
	data = NULL;
	(format == '%') ? data = (flg[spc] = 0, flg[plus] = 0, flg[prcsn] = 0, ft_strdup("%")): 0;////////
	...
	if ((((format == 'x' || format == 'X') && flg[hash]) || format == 'p' )) //&& flg[is_pri] != 1)
	{
		temp = data;
		(format == 'p') ? data = strjoin("0x", data) : 0;
		format == 'x' ? (data = strjoin("0x", data), 0)
		: format == 'X' ? (data = strjoin("0X", data), 0) : 0;
		free(temp);
	}
